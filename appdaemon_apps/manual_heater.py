...
class ManualHeater(hass.Hass):
    ...
    def initialize(self):
        ...
        for item in self.entities.input_boolean:
            ...
            if result:
                ...
                self.listen_state(self.inputBoolean, "input_boolean."+str(item), hour=result.group(1), minute=result.group(2))
    ...
    def inputBoolean(self, entity, attribute, old, new, kwargs):
        self.log("__function__@__line__: Hour is "+kwargs["hour"]+", minute is "+kwargs["minute"]+", state is "+new)
        ...
        booleanTime = datetime.datetime(year=2000, month=1, day=1, hour=int(kwargs["hour"]), minute=int(kwargs["minute"]))
    ...
    def checkManualHeater(self, kwargs):
        ...
        if currentMinute % self.interval_length == 0:
            ...
            #get data for the previous interval as well. Get the current time HH:MM and deduct an interval_length and convert it back to time
            #previousTime = self.time() - self.interval_length * 60
            #TODO

            self.controlHeater(currentState, None)
    ...
    def controlHeater(self, newstate, oldstate):
        ...
        if newstate == "on":
            ...
            if eval("self.entities." + self.heater + ".state") != "off" and thermostat_current >= thermostat_threshold:
                ...
                self.call_service("homeassistant/turn_off", entity_id=self.heater)
